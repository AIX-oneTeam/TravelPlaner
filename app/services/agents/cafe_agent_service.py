...
# LLM 초기화 (pydantic v2 사용 시 model_dump() 이용)
my_llm = LLM(
    model="gpt-4o-mini",
...
def cafe_agent(user_input, user_prompt=""):
    ...
    try:
        ...
        researcher = Agent(
            ...
            backstory="""
            사용자의 여행을 특별하게 만들기 위해, 최적의 카페를 찾고 카페의 매력을 심층 분석하여 
            사용자가 최적의 선택을 할 수 있도록 하세요.
            """,
        ...
        checker = Agent(
            ...
            backstory="researcher가 준 카페리스트를 토대로 정확한 정보를 찾아주세요",
            allow_delegation=False,
            tools=[multi_tool],
            llm=my_llm,
            verbose=True
        ...
        # 첫 번째 태스크: 카페 검색 및 분석 (연구자)
        researcher_task = Task(
            description="""
            고객이 최고의 여행을 할 수 있도록 고객의 상황과 취향에 맞는 카페를 고르기 위해 최소한 서로 다른 {n}곳 이상의 카페를 조사하고 
            최종적으로 {n}개의 카페 정보를 반환해주세요.
        ...
        checker_task = Task(
            description="""
            researcher가 반환한 카페들의 kor_name + {main_location}을 검색해, 
            spots_pydantic에 필요한 정보를 수집하세요.
            모르는 정보는 지어내지 말고 "정보 없음"으로 작성하세요.
            """,
            expected_output="""
        ...
        # Crew(멀티 에이전트 시스템) 구성: 연구자와 검증자를 순차적으로 실행
        crew = Crew(
        ...
        # 실행: CrewAI를 실행하여 태스크 결과 생성
        try:
            crew.kickoff(inputs=user_input)
            # 디버그: 원시(LiteLLM) 출력 확인
            print("researcher_task raw:", researcher_task.output.raw)
            print("checker_task raw:", checker_task.output.raw)
            
            # 최종적으로 검증 태스크의 pydantic 인스턴스를 얻어 딕셔너리로 변환
            final_pydantic_output = checker_task.output_pydantic
            if final_pydantic_output is None:
                return {"error": "pydantic parse failed"}
            return final_pydantic_output.model_dump()
