...
# 일정 저장
@router.post("")
async def create_plan(request_data: PlanRequest, request: Request, session: AsyncSession = Depends(get_session_async)):
    try:
        # 0. memberid 획득
...
# 회원의 모든 일정만 리스트 조회
@router.get("")
async def read_member_plans(request: Request, session: AsyncSession = Depends(get_session_async)):
    try:
        if(request.state.user is not None):
            member_email = request.state.user.get("email")
            member_id = await get_memberId_by_email(member_email, session)
...
# 일정 수정
@router.post("/{plan_id}")
async def update_plan(plan_id: int, request_data: PlanRequest, request: Request, session: AsyncSession = Depends(get_session_async)):
    try:
        if(request.state.user is not None):
            member_email = request.state.user.get("email")
            member_id = await get_memberId_by_email(member_email, session)
...
async def erase_plan(plan_id: int, request: Request, session: AsyncSession = Depends(get_session_async)):
    try:
        if(request.state.user is not None):
            member_email = request.state.user.get("email")
            member_id = await get_memberId_by_email(member_email, session)
