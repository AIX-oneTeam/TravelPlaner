...
# 일정 저장
@router.post("")
async def create_spot(spot: Spot, session: AsyncSession = Depends(get_session_async)):
    try:
...
# 일정 조회    
@router.get("/{spot_id}")
async def read_spot(spot_id: int, session: AsyncSession = Depends(get_session_async)):
    try:
